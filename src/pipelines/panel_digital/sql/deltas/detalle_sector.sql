WITH aggregated_delta AS (
    ...
    WHERE year >= 2024 AND (sector_id IS NOT NULL OR sector_name IS NOT NULL)
    GROUP BY management_id, management_name, sector_id, sector_name, year, month
)

MERGE [panel_digital].[detalle_sector] AS target
USING aggregated_delta AS source
    ...
    AND ISNULL(target.sector_id, -1) = ISNULL(source.sector_id, -1)
    AND ISNULL(target.sector_name, '') = ISNULL(source.sector_name, '')
